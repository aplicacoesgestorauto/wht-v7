...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const schema = Yup.object().shape({
    ...
    priority: Yup.number().required(), // Validando priority como number
    status: Yup.boolean().required() // Validando status como boolean
  });
  ...
  const record = await CreateService({
    ...data,
    companyId,
    priority: Number(data.priority), // Convertido para number
    status: Boolean(data.status) // Convertido para boolean
  });
