...
type StoreData = {
  ...
  scheduledAt: Date; // Alterado para Date
  companyId: number;
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const schema = Yup.object().shape({
    ...
    scheduledAt: Yup.date().required() // Adicionado validação para Date
  });
  ...
  if (typeof data.tagListId === 'number') {
    ...
    async function createContactListFromTag(tagId: number): Promise<number> {
    ...
    createContactListFromTag(Number(tagId)) // Convertido para number
      .then(async (contactListId) => {
        const record = await CreateService({
          ...data,
          companyId,
          contactListId: contactListId,
          scheduledAt: new Date(data.scheduledAt) // Convertido para Date
        });
        const io = getIO();
...
export const show = async (req: Request, res: Response): Promise<Response> => {
  ...
  const record = await ShowService(Number(id)); // Convertido para number
...
export const update = async (
  ...
  const record = await UpdateService({
    ...data,
    id: Number(id), // Convertido para number
    scheduledAt: new Date(data.scheduledAt) // Convertido para Date
  });
