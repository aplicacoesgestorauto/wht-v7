...
type FilterList = {
  ...
  parentId: number | null; // Alterado para number | null
};

export const index = async (req: Request, res: Response): Promise<Response> => {
  const { queueId, queueOptionId, parentId } = req.query as unknown as FilterList;

  // Convert parentId to number or null
  const parsedParentId = parentId === null || parentId === undefined ? null : Number(parentId);

  const queueOptions = await ListService({ queueId, queueOptionId, parentId: parsedParentId });
