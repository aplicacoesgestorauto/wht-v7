...
interface TicketData {
  ...
  queueId: number;
  userId: number;
  whatsappId: string; // Alterado de number para string
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  const { contactId, status, userId, queueId, whatsappId: rawWhatsappId }: {
    contactId: number;
    status: string;
    userId: number;
    queueId: number;
    whatsappId?: string; // Explicitly define as string | undefined from req.body
  } = req.body;
  const { companyId } = req.user;

  const whatsappId = rawWhatsappId ? rawWhatsappId as string : undefined; // Garante que whatsappId seja string ou undefined
  ...
  const ticket = await CreateTicketService({
    ...
    companyId,
    queueId,
    whatsappId: whatsappId // Passa como string ou undefined
  });
...
export const update = async (
  ...
  const { ticket } = await UpdateTicketService({
    ticketData: {
      ...ticketData,
      whatsappId: ticketData.whatsappId ? Number(ticketData.whatsappId) : undefined // Convertido para number
    },
    ticketId,
    companyId
  });
