...
import { verifyMessage } from "../WbotServices/wbotHelpers";
...
const UpdateTicketService = async ({
  ...
  try {
    ...
    const ticketTraking = await FindOrCreateATicketTrakingService({
      ticketId: Number(ticketId), // Explicitly cast to number
      companyId,
    ...
    if (oldStatus === "closed" || (whatsappId && Number(whatsappId) !== ticket.whatsappId)) {
      ...
      // }
      await CheckContactOpenTickets(ticket.contact.id, String(whatsappId));
      chatbot = null;
      queueOptionId = null;
    }
